  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      gameState.trees.forEach((tree) => {
        ...
        if (tree.thirst > 60 && tree.stage !== "dry") {
          const dropBob = Math.sin(Date.now() / 300 + tree.x) * 2
          ctx.fillStyle = "#06b6d4"
          ctx.font = "bold 16px Arial"
          ctx.textAlign = "center"
          ctx.fillText("💧", tree.x - cameraX + sway, tree.y - cameraY - treeSize - 8 + dropBob)
        }
      })
      ...
      gameState.animals.forEach((animal) => {
        ...
        const bounce = isMoving
          ? Math.abs(Math.sin(Date.now() / 200 + animal.x)) * 3
          : Math.abs(Math.sin(Date.now() / 300 + animal.x)) * 1
        ...
        ctx.beginPath()
        ctx.arc(
          animal.x - cameraX + bodySize * 0.6,
          animal.y - cameraY - bodySize * 0.3 - bounce,
          bodySize * 0.4,
          0,
          Math.PI * 2,
        )
      ...
      if (gameState.fires.length > 0) {
        const closestFire = gameState.fires.reduce((closest, fire) => {
          const dist = Math.hypot(fire.x - gameState.playerPos.x, fire.y - gameState.playerPos.y)
          const closestDist = closest
            ? Math.hypot(closest.x - gameState.playerPos.x, closest.y - gameState.playerPos.y)
            : Number.POSITIVE_INFINITY
          return dist < closestDist ? fire : closest
        }, null)

        if (closestFire) {
          const dx = closestFire.x - gameState.playerPos.x
          const dy = closestFire.y - gameState.playerPos.y
          const angle = Math.atan2(dy, dx)
          const distance = Math.hypot(dx, dy)

          // Only show arrow if fire is off-screen or far away
          if (distance > 100) {
            const arrowDistance = 50
            const arrowX = canvasWidth / 2 + Math.cos(angle) * arrowDistance
            const arrowY = canvasHeight / 2 + Math.sin(angle) * arrowDistance
            ...
            ctx.fillStyle = "#ef4444"
            ctx.beginPath()
            ctx.moveTo(pulse * 0.6, 0)
            ctx.lineTo(-pulse * 0.3, -pulse * 0.3)
            ctx.lineTo(-pulse * 0.3, pulse * 0.3)
            ctx.closePath()
            ctx.fill()
            ...
            ctx.textAlign = "center"
            ctx.strokeText(`${Math.round(distance)}m`, arrowX, arrowY - 35)
            ctx.fillText(`${Math.round(distance)}m`, arrowX, arrowY - 35)

            // Fire icon
            ctx.font = "bold 20px Arial"
            ctx.strokeText("🔥", arrowX, arrowY - 50)
            ctx.fillText("🔥", arrowX, arrowY - 50)
          }
        }
      }
